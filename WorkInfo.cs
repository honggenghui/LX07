   ...
   public  class WorkInfo
    {
        ...
        public Model.WorkInfo GetModel(int id)
        {
            ...
            {
                string sql = "select workinfo.* ,activity.activityName from workinfo join activity on workinfo.activity=activityId where workId=@id" ;
                return cn.QueryFirstOrDefault<Model.WorkInfo>(sql, new { id=id });
            }
        }
        ...
        public IEnumerable<Model.WorkInfo> GetPage(Model.WorkPage page)
        {
            using (IDbConnection cn = new MySqlConnection(cns))
            {
                string sql = "with a as(select row_number() over (order by uploadTime desc) as num,workinfo.* from workinfo where activity in @activityIds  ";
        ...
        public IEnumerable<Model.WorkInfoNo> GetFindPage(Model.WorkFindPage page)
        {
            using (IDbConnection cn = new MySqlConnection(cns))
            {
                ...
                sql += "select * from a where between(@pageIndex-1)*@pageSize+1 and @pageIndex*@pageSize;";
        ...
        public int Delete(int id)
        {
            using (IDbConnection cn = new MySqlConnection(cns))
            {
                string sql = "  delete from workinfo where workif=@id ";
